...
export async function createUserSessionHandler(req:Request, res:Response){
    ...
    if(!user){
        ...
        const accessToken = signJwt({...user, session: session._id} ,"accessPrivateKey",{expiresIn: config.accessTokenTtl})

        const refreshToken = signJwt({...user, session: session._id} ,"refreshPrivateKey",{expiresIn: config.refreshTokenTtl})
